...
if __name__ == '__main__':
    ...
    # Load data
    train_dl = create_dl(
        paths=list((Path(args.data_dir) / 'train').iterdir()),
    ...
    valid_dl = create_dl(
        paths=list((Path(args.data_dir) / 'valid').iterdir()),
    ...
    history = train_loop(
        model, train_dl, valid_dl, optimizer, criterion, args.epochs, args.model_dir
    )
